...
int main() {
    ...
    Dimension M(1,0,0), L(0,1,0), T(0,0,1);
    units["kg"] = Unit("kg", 1.0, M);
    units["m"]  = Unit("m", 1.0, L);
    units["s"]  = Unit("s", 1.0, T);
    units["N"]  = Unit("N", 1.0, Dimension(1,1,-2)); // Ньютон

    std::cout << "Calculate" << std::endl;
    std::cout << "Пример: 10 kg * 5 m / 2 s" << std::endl;

    std::string line;
    while (std::getline(std::cin, line) && line != "exit") {
        try {
            Parser p(line, units);
            Quantity res = p.parse();
            std::cout << "Result SI: ";
            res.printSI();
        } catch (const std::exception& e) {
